...
def update_motor_params():
    ...
    FBMDeg.motor_controller.move_and_lock_motors(
        hip_position, knee_position, hip_velocity_scale, knee_velocity_scale, lock_duration, hip_force_threshold, knee_force_threshold, user_weight
    )
...
# User Weight
tk.Label(root, text="User Weight (kg):").grid(row=7, column=0, padx=10, pady=5)
user_weight_entry = tk.Entry(root)
user_weight_entry.grid(row=7, column=1, padx=10, pady=5)
