...
class MotorController(Node):
    def __init__(self, epos_hip, epos_knee, force_based_mode=False):
        ...
        self.hip_gravity_buffer = deque(maxlen=70)  # buffer for moving average of gravity for hip
        self.knee_gravity_buffer = deque(maxlen=70)  # buffer for moving average of gravity for knee
    ...
    def force_callback(self, msg):
        ...
        # Get current positions for gravity compensation calculations
        current_position_hip = self.motor_hip.get_position_Degree()
        current_position_knee = self.motor_knee.get_position_Degree()

        # Add gravity compensation
        gravity_comp_hip = self.calculate_gravity_hip(current_position_hip, current_position_knee)
        gravity_comp_knee = self.calculate_gravity_knee(current_position_knee)

        sum_force_hip += gravity_comp_hip
        sum_force_knee += gravity_comp_knee
    ...
    def calculate_gravity_hip(self, theta_1, theta_2):
        g = 9.81  # gravitational acceleration
        m1 = 2.95  # mass of link 1 in kg
        m2 = 3.80  # mass of link 2 in kg
        L1 = 0.42  # length of link 1 in meters
        L2 = 0.54  # length of link 2 in meters
        user_weight_hip = (self.user_weight / 2) * 0.6  # 60% of user weight the division by 2 is giving us equal distribution on hip and knee
        user_weight_knee = (self.user_weight / 2) * 0.4  # 40% of user weight the division by 2 is giving us equal distribution on hip and knee

        tau_g1 = -0.5 * L1 * (m1 + user_weight_hip) * g * cos(theta_1)
        tau_g2 = -(m2 + user_weight_knee) * g * (L1 * cos(theta_1) + 0.5 * L2 * cos(theta_1 - theta_2))

        gravity_hip = tau_g1 + tau_g2

        # Update moving average buffer
        self.hip_gravity_buffer.append(gravity_hip)
        moving_average_hip = sum(self.hip_gravity_buffer) / len(self.hip_gravity_buffer)

        return moving_average_hip

    def calculate_gravity_knee(self, theta_2):
        g = 9.81  # gravitational acceleration
        m2 = 3.80  # mass of link 2 in kg
        L2 = 0.54  # length of link 2 in meters
        user_weight_knee = self.user_weight * 0.4  # 40% of user weight

        tau_g2 = -0.5 * L2 * (m2 + user_weight_knee) * g * cos(theta_2)

        gravity_knee = tau_g2

        # Update moving average buffer
        self.knee_gravity_buffer.append(gravity_knee)
        moving_average_knee = sum(self.knee_gravity_buffer) / len(self.knee_gravity_buffer)

        return moving_average_knee
    ...
    def move_and_lock_motors(self, hip_position, knee_position, hip_velocity_scale, knee_velocity_scale, lock_duration, hip_force_threshold, knee_force_threshold, user_weight):
        ...
        print(f"Set user weight: {user_weight} kg")
